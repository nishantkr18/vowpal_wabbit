...
void drain_examples(vw& all)
{
  ...
  {  // drain any extra examples from parser.
    example* ec = nullptr;
    while ((ec = VW::get_example(all, all.example_parser)) != nullptr) VW::finish_example(all, *ec);
  }
  all.l->end_examples();
}
...
public:
  ...
  example* pop() { return !_master.early_terminate ? VW::get_example(_master, _master.example_parser) : nullptr; }
